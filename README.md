...
间隙对齐到**落位栅格** = 序列帧格（`sequence.timebase`）与采样栅格的最小公倍数。
Premiere 会把剪辑位置吸附到序列帧格上，所以间隙必须落在帧格上 —— 否则算出来的位置会被
吸附，实测间隙会互相差最多一帧。可用空隙切成整数个落位栅格单位后，除不尽的余数会**均匀
摊到各个间隙上**（每个多一格），因此所有间隙只差一格（本工程序列里是 18 帧 / 19 帧，
相差约 16.7 毫秒），不会在末尾留下大窟窿。

序列的 `timebase` 读不到时脚本会中止，不会退化成"只对齐采样栅格"再被 Premiere 吸附。
